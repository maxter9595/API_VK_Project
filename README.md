...
Далее отправляем фотографии по URL методом POST на папку, заданную пользователем в методе ```TOTAL_upload_VK_photo``` (по умолчению фотографии отправляются на папку VK_Images). Результат - создание (при необходимости) и заполнение выбранной папки в Яндекс Диске.
